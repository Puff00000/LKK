...
export default function LocalDashboard() {
  ...
  const handleAccept = async (bookingId) => {
    ...
    try {
      ...
      setBookings((prev) => prev.map((b) => b.id === bookingId ? { ...b, status: "awaiting_payment" } : b));
    } catch (e) {
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {isRequested && (
                        <div className="mt-2 text-xs text-amber-700">
                          ⏰ Respond within 24h, or this request expires automatically.
                        </div>
                      )}
                      {isAwaitingPayment && (
                        <div className="mt-2 text-xs text-amber-700">
                          ⏰ Waiting on the traveller to pay — releases back to you automatically after 24h if they don't.
                        </div>
                      )}
                    </div>

                    {/* Accept/Decline — only while this is still a bare request.
                        Once accepted, payment is the traveller's move next; the
                        local can't unilaterally back out here anymore. */}
                    {isRequested && (
